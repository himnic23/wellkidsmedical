const translations = {
    en: {
        vaccine: {
            kicker: "Vaccination Planning With Clear Guidance",
            title: "Childhood Vaccination Program",
            intro: "Well Kids Medical Centre provides comprehensive childhood vaccination services to ensure your child receives complete vaccine protection. Below are our vaccination programs, including the Government Health Clinic Vaccination Program and the Recommended Vaccination Program - Enhanced Protection.",
            note: "Families can use this page as a reference, then contact the clinic for age-specific advice and scheduling.",
            contact_title: "Need a vaccine enquiry?",
            contact_text: "Call for appointment booking or WhatsApp us with your child's age for guidance.",
            importance_title: "Importance of Vaccination",
            importance_text1: "Vaccination is one of the most effective methods of disease prevention, especially important for children. By getting vaccinated, it helps children's immune systems identify and resist specific pathogens, thereby preventing various serious diseases.",
            importance_text2: "Our pediatric specialists will provide personalized vaccination recommendations based on your child's age, health condition, and needs. If you have any questions, please feel free to contact us.",
            table: {
                age_group: "Age Group",
                govt_program: "Government Health Clinic Vaccination Program",
                enhanced_program: "Recommended Vaccination Program - Enhanced Protection",
                newborn: "Newborn",
                one_month: "1 Month",
                two_months: "2 Months",
                four_months: "4 Months",
                six_months: "6 Months",
                nine_months: "9 Months",
                one_year: "1 Year",
                fifteen_months: "15 Months",
                eighteen_months: "18 Months",
                two_years: "2 Years",
                bcg: "BCG",
                hepb: "Hepatitis B Vaccine",
                dtap_ipv: "DTaP-IPV",
                pcv: "Pneumococcal Conjugate Vaccine",
                mmr: "Measles, Mumps, Rubella Vaccine",
                varicella: "Varicella Vaccine",
                six_in_one: "6-in-1 Vaccine",
                five_in_one: "5-in-1 Vaccine",
                rotavirus: "Rotavirus Vaccine",
                je: "Japanese Encephalitis Vaccine",
                menb: "Meningococcal B Vaccine",
                mmrv: "Measles, Mumps, Rubella, Varicella Vaccine",
                menc: "Meningococcal ACWY Vaccine",
                hepa: "Hepatitis A Vaccine",
                booster: "Booster",
                first_dose: "1st Dose",
                second_dose: "2nd Dose",
                third_dose: "3rd Dose",
                fourth_dose: "4th Dose",
                included: "(Not required - Already included in 6-in-1 vaccine)"
            }
        },
        footer: {
            address: {
                title: "Address",
                street: "Room 807, Argyle Centre, 688 Nathan Road, Mong Kok, Kowloon",
            },
            appointment: {
                title: "Appointment Enquiry",
                phone: "+852 23213318",
                email: "wellkidsmc@gmail.com"
            },
            social: {
                title: "Follow Us",
                facebook: "Facebook",
                instagram: "Instagram",
                whatsapp: "Scan WhatsApp QR"
            }
        },
        home: {
            hero: {
                title: "Well Kids Medical Centre",
                kicker: "Family-Focused Specialist Care",
                subtitle: "Well Kids Medical Centre provides pediatric specialist outpatient services, neonatal jaundice treatment, emergency delivery visit services, newborn hospitalization rounds services, private hospital hospitalization arrangements.",
                point1: "Specialist paediatric care for newborns, infants, children, and adolescents",
                point2: "Vaccination support, developmental assessment, and hospital arrangements",
                point3: "Warm clinic environment with clear appointment and enquiry channels"
            },
            quick: {
                title: "Quick Contact",
                phone_note: "Best for appointments and urgent enquiries during clinic hours.",
                hours_note: "Monday to Friday: 10:00 AM - 6:00 PM\nSaturday: 10:00 AM - 1:00 PM",
                location_note: "Convenient Mong Kok location"
            },
            answers: {
                kicker: "Verified Clinic Facts",
                title: "Quick Answers Parents Often Ask",
                intro: "These short answers help families and search tools quickly understand what the clinic offers.",
                item1_title: "Clinic Type",
                item1_text: "Family-friendly paediatric specialist clinic serving newborns, infants, children, and adolescents in Hong Kong.",
                item2_title: "Location",
                item2_text: "The clinic is in Mong Kok, Kowloon, at Room 807, Argyle Centre, 688 Nathan Road.",
                item3_title: "How to Book",
                item3_text: "Phone is best for appointment booking and urgent enquiries during clinic hours. WhatsApp is available for general enquiries.",
                item4_title: "Key Services",
                item4_text: "Paediatric consultations, childhood vaccinations, developmental assessment, breastfeeding support, speech therapy, and newborn or hospital support."
            },
            faq: {
                kicker: "Common Questions",
                title: "Useful Questions About the Clinic",
                q1: "Where is Well Kids Medical Centre located?",
                a1: "We are located at Room 807, Argyle Centre, 688 Nathan Road, Mong Kok, Kowloon.",
                q2: "How can parents contact the clinic?",
                a2: "Parents can call +852 23213318 for appointments and use WhatsApp for general enquiries.",
                q3: "What services does the clinic provide?",
                a3: "The clinic provides paediatric specialist consultations, vaccination services, developmental assessment, breastfeeding support, speech therapy, and newborn or inpatient support.",
                q4: "Who are the doctors at the clinic?",
                a4: "The clinic's medical team includes Dr. So Wing Yee and Dr. Tsang Chi Ho, both specialists in paediatrics."
            },
            pathways: {
                services_title: "Services",
                services_desc: "Explore vaccination, breastfeeding, speech therapy, and inpatient support.",
                doctors_title: "Doctors",
                doctors_desc: "Meet our paediatric specialists and review their clinical credentials.",
                contact_title: "Contact",
                contact_desc: "Call or WhatsApp us for appointments, directions, and general enquiries."
            },
            features: {
                expertise: {
                    title: "Medical Expertise",
                    description: "Our team of experienced doctors provides comprehensive medical care."
                },
                care: {
                    title: "Patient Care",
                    description: "We prioritize patient comfort and personalized treatment plans."
                },
                facilities: {
                    title: "Modern Facilities",
                    description: "State-of-the-art medical equipment and comfortable environment."
                }
            }
        },
        services: {
            kicker: "Care Designed Around Families",
            title: "Our Pediatric Services",
            description: "We offer a variety of pediatric medical services, including:",
            lead_title: "Professional care for every stage of childhood",
            lead_description: "We organise our care around daily family needs: clinic visits, vaccinations, newborn support, developmental follow-up, and private hospital arrangements.",
            contact_title: "Need help choosing the right service?",
            contact_description: "Call the clinic for appointment booking or WhatsApp us for general service enquiries.",
            general_medicine: "Hong Kong Pediatric Specialist Clinic Services",
            general_medicine_desc: "Developmental assessment, school entry physical examination, allergy testing, allergen desensitization treatment, fever, and illness.",
            pediatrics: "Vaccination",
            pediatrics_desc: "Newborn babies, supplementary vaccines required in Hong Kong for foreign-born babies, and vaccines for studying abroad. We provide Department of Health designated vaccines for infants and school-age children, as well as additional preventive injections such as Japanese encephalitis, hepatitis A, meningitis, rotavirus, Haemophilus influenzae type b, etc.",
            cardiology: "Breastfeeding Consultant",
            cardiology_desc: "Internationally certified lactation consultants with knowledge of breastfeeding and extensive guidance experience to assist new mothers in breastfeeding with the correct posture.",
            dermatology: "Speech Therapy",
            dermatology_desc: "Speech therapists provide assessment and treatment for children with language development delays, pronunciation or voice disorders, and oral muscle problems.",
            orthopedics: "Inpatient Services",
            orthopedics_desc: "Newborn inpatient ward rounds, private hospital admission arrangements."
        },
        nav: {
            home: 'Home',
            about: 'About',
            services: 'Services',
            doctors: 'Doctors',
            contact: 'Contact',
            cta_call: 'Call Clinic',
            cta_whatsapp: 'WhatsApp Enquiry'
        },
        doctors: {
            title: 'Our Medical Team',
            kicker: 'Specialist Paediatric Team',
            intro: 'Our doctors combine specialist paediatric training with practical newborn, vaccination, and inpatient care support for families in Hong Kong.',
            cta: 'Contact Doctor',
            doctor1: {
                name: 'Dr. So Wing Yee',
                specialty: 'Specialist Paediatrics',
                credentials: 'MBBS(HK)\nMRCPCH\nFHKCPaed\nFHKAM(Paediatrics)\nDCH(International)(RCPCH)\nDCH(HK)(HKCPaed)'
            },
            doctor2: {
                name: 'Dr. Tsang Chi Ho',
                specialty: 'Specialist Paediatrics',
                credentials: 'MBBS(HK)\nMRCPCH\nFHKCPaed\nFHKAM(Paediatrics)\nDCH(International)(RCPCH)\nDCH(HK)(HKCPaed)'
            }
        },
        about: {
            title: "About Well Kids Medical Centre",
            kicker: "Family-Friendly. Professional. Trustworthy.",
            description: "Well Kids Medical Centre is dedicated to providing exceptional pediatric healthcare services with a child-friendly approach. Our team of experienced pediatric professionals strives to deliver the highest quality care in a comfortable and welcoming environment.",
            mission: "Our Mission",
            mission_text: "To provide exceptional healthcare services with compassion and expertise.",
            vision: "Our Vision",
            vision_text: "To be the leading healthcare provider, recognized for excellence in patient care.",
            highlight1_title: "Specialist Paediatrics",
            highlight1_desc: "Professional paediatric consultation and follow-up care for families who want clear, dependable advice.",
            highlight2_title: "Support for Growing Families",
            highlight2_desc: "From newborn concerns to vaccination planning, we help parents navigate each stage with confidence.",
            highlight3_title: "Convenient Enquiry Channels",
            highlight3_desc: "Appointments and general enquiries are easy to start by phone or WhatsApp."
        },
        contact: {
            kicker: 'Appointments & Enquiries',
            title: 'Contact Us',
            intro: "We're here to help with your child's healthcare needs. Feel free to reach out to us through any of the following channels:",
            booking: {
                title: 'Need to make an appointment?',
                description: 'Call us during clinic hours or send an email any time. We will help you arrange a suitable consultation slot.'
            },
            address: {
                title: 'Address',
                value: "Room 807, Argyle Centre, 688 Nathan Road, Mong Kok, Kowloon"
            },
            phone: {
                title: 'Phone',
                value: '+852 23213318'
            },
            email: {
                title: 'Email',
                value: 'wellkidsmc@gmail.com'
            },
            hours: {
                title: 'Operating Hours',
                value: 'Monday - Friday: 10:00 AM - 6:00 PM\nSaturday: 10:00 AM - 1:00 PM\nSunday and Public Holiday: Closed'
            }
        }
    },
    zh: {
        vaccine: {
            kicker: "以清晰建議協助家長安排疫苗接種",
            title: "兒童疫苗計劃",
            intro: "允翹兒科中心提供全面的兒童疫苗接種服務，確保您的孩子獲得完整的疫苗保護。以下是我們的疫苗接種計劃，包括政府健康院疫苗接種計劃和建議的疫苗接種計劃-加強保護。",
            note: "家長可先以此頁作參考，再聯絡診所了解適合孩子年齡的安排及接種時間。",
            contact_title: "想查詢疫苗安排？",
            contact_text: "歡迎致電預約，或以 WhatsApp 提供孩子年齡作一般查詢。",
            importance_title: "疫苗接種的重要性",
            importance_text1: "疫苗接種是預防疾病最有效的方法之一，對兒童尤為重要。通過接種疫苗，有助於兒童的免疫系統識別和抵抗特定病原體，從而預防各種嚴重疾病。",
            importance_text2: "我們的兒科專家將根據您孩子的年齡、健康狀況和需求，提供個性化的疫苗接種建議。如有任何疑問，請隨時與我們聯繫。",
            table: {
                age_group: "年齡組別",
                govt_program: "政府健康院疫苗計劃",
                enhanced_program: "建議的疫苗計劃-加強保護",
                newborn: "初生",
                one_month: "一個月",
                two_months: "兩個月",
                four_months: "四個月",
                six_months: "六個月",
                nine_months: "九個月",
                one_year: "一歲",
                fifteen_months: "十五個月",
                eighteen_months: "十八個月",
                two_years: "兩歲",
                bcg: "卡介苗",
                hepb: "乙型肝炎疫苗",
                dtap_ipv: "四合一疫苗",
                pcv: "肺炎球菌疫苗",
                mmr: "麻疹、腮腺炎、德國麻疹混合疫苗",
                varicella: "水痘疫苗",
                six_in_one: "六合一疫苗",
                five_in_one: "五合一疫苗",
                rotavirus: "輪狀病毒疫苗",
                je: "日本腦炎疫苗",
                menb: "腦膜炎雙球菌疫苗 B型",
                mmrv: "麻疹、腮腺炎、德國麻疹及水痘混合疫苗",
                menc: "腦膜炎雙球菌疫苗 ACWY型",
                hepa: "甲型肝炎疫苗",
                booster: "加強劑",
                first_dose: "第1次",
                second_dose: "第2次",
                third_dose: "第3次",
                fourth_dose: "第4次",
                included: "(不用打，已包含在六合一疫苗)"
            }
        },
        footer: {
            address: {
                title: "地址",
                street: "旺角彌敦道688號旺角中心807室"
            },
            appointment: {
                title: "預約查詢",
                phone: "+852 23213318",
                email: "wellkidsmc@gmail.com"
            },
            social: {
                title: "關注我們",
                facebook: "Facebook",
                instagram: "Instagram",
                whatsapp: "WhatsApp掃碼"
            }
        },
        about: {
            title: "關於允翹兒科中心",
            kicker: "親切照護與專業醫療並重",
            description: "允翹兒科中心致力於以兒童友好的方式提供卓越的兒科醫療服務。我們經驗豐富的兒科專業團隊努力在舒適和溫馨的環境中提供最優質的護理。",
            mission: "我們的使命",
            mission_text: "以同理心和專業知識提供卓越的醫療服務。",
            vision: "我們的願景",
            vision_text: "成為以患者護理卓越著稱的領先醫療服務提供者。",
            highlight1_title: "兒科專科照護",
            highlight1_desc: "為重視專業判斷及清晰建議的家庭，提供可靠的兒科門診及跟進服務。",
            highlight2_title: "陪伴家庭每個成長階段",
            highlight2_desc: "由新生嬰兒到兒童疫苗及發展需要，我們協助家長更安心地照顧孩子。",
            highlight3_title: "查詢及預約更方便",
            highlight3_desc: "可透過電話及 WhatsApp 快速開始預約及一般查詢。"
        },
        home: {
            hero: {
                title: "允翹兒科中心",
                kicker: "以家庭為本的兒科專科照護",
                subtitle: "允翹兒科中心提供兒科專科門診服務、新生嬰兒黃疸治療、緊急分娩到診服務、初生嬰兒住院巡房服務 (浸會醫院、聖德肋撒醫院(法國醫院))、私家醫院住院安排 (聖德肋撒醫院(法國醫院))",
                point1: "為初生嬰兒、幼兒、兒童及青少年提供兒科專科照護",
                point2: "疫苗接種、發展評估、住院安排及母乳支援服務",
                point3: "溫和親切的診所環境，並提供清晰方便的預約及查詢渠道"
            },
            quick: {
                title: "快速聯絡",
                phone_note: "診症預約及較緊急的查詢，建議於診症時間內致電。",
                hours_note: "星期一至五：上午10:00 - 下午6:00\n星期六：上午10:00 - 下午1:00",
                location_note: "九龍旺角"
            },
            answers: {
                kicker: "診所重點資料",
                title: "家長常問的快速答案",
                intro: "以下重點資料有助家長及搜尋工具更快了解本中心的服務及聯絡方式。",
                item1_title: "診所類型",
                item1_text: "以家庭為本的兒科專科診所，為初生嬰兒、幼兒、兒童及青少年提供醫療服務。",
                item2_title: "診所位置",
                item2_text: "診所位於九龍旺角彌敦道688號旺角中心807室，交通方便。",
                item3_title: "如何預約",
                item3_text: "預約及較緊急查詢建議於診症時間內致電；一般問題可透過 WhatsApp 聯絡。",
                item4_title: "主要服務",
                item4_text: "提供兒科專科門診、兒童疫苗、發展評估、母乳支援、言語治療及新生嬰兒或住院支援服務。"
            },
            faq: {
                kicker: "常見問題",
                title: "關於本中心的常用問題",
                q1: "允翹兒科中心位於哪裡？",
                a1: "本中心位於九龍旺角彌敦道688號旺角中心807室。",
                q2: "家長可如何聯絡診所？",
                a2: "家長可致電 +852 23213318 預約診症，亦可透過 WhatsApp 作一般查詢。",
                q3: "本中心提供哪些服務？",
                a3: "本中心提供兒科專科門診、疫苗接種、發展評估、母乳支援、言語治療，以及新生嬰兒或住院支援服務。",
                q4: "本中心有哪些醫生？",
                a4: "本中心醫療團隊包括蘇詠怡醫生及曾志豪醫生，兩位均為兒科專科醫生。"
            },
            pathways: {
                services_title: "服務項目",
                services_desc: "了解疫苗、母乳顧問、言語治療及住院支援服務。",
                doctors_title: "醫生團隊",
                doctors_desc: "認識本中心兒科專科醫生及其臨床資歷。",
                contact_title: "聯絡我們",
                contact_desc: "可致電或以 WhatsApp 查詢預約、地址及一般問題。"
            },
            features: {
                expertise: {
                    title: "醫療專長",
                    description: "我們經驗豐富的醫生團隊提供全面的醫療服務。"
                },
                care: {
                    title: "患者護理",
                    description: "我們注重患者舒適度及個性化治療方案。"
                },
                facilities: {
                    title: "現代設施",
                    description: "先進的醫療設備和舒適的環境。"
                }
            }
        },
        services: {
            kicker: "以家庭需要為中心的兒科服務",
            title: "兒科服務",
            description: "我們有多種兒童醫療服務，包括：",
            lead_title: "按孩子成長階段提供合適照護",
            lead_description: "我們的服務圍繞家庭日常需要而設，包括門診診治、疫苗接種、新生嬰兒支援、發展跟進及私家醫院住院安排。",
            contact_title: "不確定應預約哪項服務？",
            contact_description: "歡迎致電診所安排預約，或以 WhatsApp 作一般服務查詢。",
            general_medicine: "香港兒科專科門診服務",
            general_medicine_desc: "發展評估、入學身體檢查、過敏檢測、脫敏治療、發燒、疾病。",
            pediatrics: "疫苗接種",
            pediatrics_desc: "新生嬰兒、外國出生嬰兒補打香港需要疫苗、外國升學疫苗接種。提供衞生署指定嬰幼兒及適齡學童接種疫苗及額外預防針，如日本腦炎、甲型肝炎、腦膜炎、輪狀病毒、乙型嗜血性流感桿菌等。",
            cardiology: "母乳顧問",
            cardiology_desc: "由擁有母乳餵哺知識和豐富指導經驗的國際認證泌乳顧問，輔助新手媽媽以正確姿勢餵哺母乳。",
            dermatology: "言語治療",
            dermatology_desc: "由言語治療師為有語言發展遲緩、發音或聲線障礙、口部肌肉等問題的兒童提供評估與治療。",
            orthopedics: "住院服務",
            orthopedics_desc: "初生嬰兒住院巡房服務，私家醫院住院安排。"
        },
        nav: {
            home: '首頁',
            about: '關於我們',
            services: '服務',
            doctors: '醫生團隊',
            contact: '聯絡我們',
            cta_call: '致電診所',
            cta_whatsapp: 'WhatsApp 查詢'
        },
        doctors: {
            title: '我們的醫療團隊',
            kicker: '兒科專科醫生團隊',
            intro: '本中心醫生團隊具備兒科專科訓練，並提供新生嬰兒、疫苗接種及住院支援等臨床服務，協助家庭安心照顧孩子。',
            cta: '聯絡醫生',
            doctor1: {
                name: '蘇詠怡醫生',
                specialty: '兒科專科醫生',
                credentials: '香港大學内外全科醫學士\n英國皇家兒科醫學院院員\n香港兒科醫學院院士\n香港醫學專科學院院士（兒科）\n英國皇家兒科醫學院兒科文憑（國際）\n香港兒科醫學院兒科文憑（香港）'
            },
            doctor2: {
                name: '曾志豪醫生',
                specialty: '兒科專科醫生',
                credentials: '香港大學内外全科醫學士\n英國皇家兒科醫學院院員\n香港兒科醫學院院士\n香港醫學專科學院院士（兒科）\n英國皇家兒科醫學院兒科文憑（國際）\n香港兒科醫學院兒科文憑（香港）'
            }
        },
        contact: {
            kicker: '預約及查詢',
            title: '聯絡我們',
            intro: '我們隨時為您的孩子提供醫療保健服務。您可以透過以下方式聯絡我們：',
            booking: {
                title: '想安排預約或查詢診症？',
                description: '歡迎於診症時間內致電，或隨時透過電郵與我們聯絡，我們會協助安排合適的應診時間。'
            },
            address: {
                title: '地址',
                value: '九龍旺角彌敦道688號旺角中心807室'
            },
            phone: {
                title: '電話',
                value: '+852 23213318'
            },
            email: {
                title: '電子郵件',
                value: 'wellkidsmc@gmail.com'
            },
            hours: {
                title: '營業時間',
                value: '星期一至五：上午10:00 - 下午6:00\n星期六：上午10:00 - 下午1:00\n星期日及公眾假期：休息'
            }
        }
    }
};
