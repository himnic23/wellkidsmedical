const translations = {
    en: {
        vaccine: {
            title: "Childhood Vaccination Program",
            intro: "Well Kids Medical Centre provides comprehensive childhood vaccination services to ensure your child receives complete vaccine protection. Below are our vaccination programs, including the Government Health Clinic Vaccination Program and the Recommended Vaccination Program - Enhanced Protection.",
            importance_title: "Importance of Vaccination",
            importance_text1: "Vaccination is one of the most effective methods of disease prevention, especially important for children. By getting vaccinated, it helps children's immune systems identify and resist specific pathogens, thereby preventing various serious diseases.",
            importance_text2: "Our pediatric specialists will provide personalized vaccination recommendations based on your child's age, health condition, and needs. If you have any questions, please feel free to contact us.",
            table: {
                age_group: "Age Group",
                govt_program: "Government Health Clinic Vaccination Program",
                enhanced_program: "Recommended Vaccination Program - Enhanced Protection",
                newborn: "Newborn",
                one_month: "1 Month",
                two_months: "2 Months",
                four_months: "4 Months",
                six_months: "6 Months",
                nine_months: "9 Months",
                one_year: "1 Year",
                fifteen_months: "15 Months",
                eighteen_months: "18 Months",
                two_years: "2 Years",
                bcg: "BCG",
                hepb: "Hepatitis B Vaccine",
                dtap_ipv: "DTaP-IPV",
                pcv: "Pneumococcal Conjugate Vaccine",
                mmr: "Measles, Mumps, Rubella Vaccine",
                varicella: "Varicella Vaccine",
                six_in_one: "6-in-1 Vaccine",
                five_in_one: "5-in-1 Vaccine",
                rotavirus: "Rotavirus Vaccine",
                je: "Japanese Encephalitis Vaccine",
                menb: "Meningococcal B Vaccine",
                mmrv: "Measles, Mumps, Rubella, Varicella Vaccine",
                menc: "Meningococcal ACWY Vaccine",
                hepa: "Hepatitis A Vaccine",
                booster: "Booster",
                first_dose: "1st Dose",
                second_dose: "2nd Dose",
                third_dose: "3rd Dose",
                fourth_dose: "4th Dose",
                included: "(Not required) - Already included in 6-in-1 vaccine"
            }
        },
        footer: {
            address: {
                title: "Address",
                street: "Room 807, Argyle Centre, 688 Nathan Road, Mong Kok, Kowloon",
            },
            appointment: {
                title: "Appointment Enquiry",
                phone: "+852 23213318",
                email: "wellkidsmc@gmail.com"
            },
            social: {
                title: "Follow Us",
                facebook: "Facebook",
            instagram: "Instagram",
            twitter: "Twitter",
            whatsapp: "WhatsApp掃碼"
            }
        },
        home: {
            hero: {
                title: "Well Kids Medical Centre",
                subtitle: "Your Health is Our Priority"
            },
            features: {
                expertise: {
                    title: "Medical Expertise",
                    description: "Our team of experienced doctors provides comprehensive medical care."
                },
                care: {
                    title: "Patient Care",
                    description: "We prioritize patient comfort and personalized treatment plans."
                },
                facilities: {
                    title: "Modern Facilities",
                    description: "State-of-the-art medical equipment and comfortable environment."
                }
            }
        },
        services: {
            title: "Our Pediatric Services",
            description: "We offer a variety of pediatric medical services, including:",
            general_medicine: "Hong Kong Pediatric Specialist Clinic Services",
            general_medicine_desc: "Developmental assessment, school entry physical examination, allergy testing, allergen desensitization treatment, fever, and illness.",
            pediatrics: "Vaccination",
            pediatrics_desc: "Newborn babies, supplementary vaccines required in Hong Kong for foreign-born babies, and vaccines for studying abroad. We provide Department of Health designated vaccines for infants and school-age children, as well as additional preventive injections such as Japanese encephalitis, hepatitis A, meningitis, rotavirus, Haemophilus influenzae type b, etc.",
            cardiology: "Breastfeeding Consultant",
            cardiology_desc: "Internationally certified lactation consultants with knowledge of breastfeeding and extensive guidance experience to assist new mothers in breastfeeding with the correct posture.",
            dermatology: "Speech Therapy",
            dermatology_desc: "Speech therapists provide assessment and treatment for children with language development delays, pronunciation or voice disorders, and oral muscle problems.",
            orthopedics: "Inpatient Services",
            orthopedics_desc: "Newborn inpatient ward rounds, private hospital admission arrangements."
        },
        nav: {
            home: 'Home',
            about: 'About',
            services: 'Services',
            doctors: 'Doctors',
            contact: 'Contact'
        },
        doctors: {
            title: 'Our Medical Team',
            doctor1: {
                name: 'Dr. So Wing Yee',
                specialty: 'Specialist Paediatrics',
                credentials: 'MBBS(HK),MRCPCH,FHKCPaed,FHKAM(Paediatrics),\nDCH(International) (RCPCH)DCH(HK)(HKCPaed)'
            },
            doctor2: {
                name: 'Dr. Tsang Chi Ho',
                specialty: 'Specialist Paediatrics',
                credentials: 'MBBS(HK),MRCPCH,FHKCPaed,FHKAM(Paediatrics),\nDCH(International)(RCPCH)DCH(HK)(HKCPaed)'
            }
        },
        about: {
            title: "About Well Kids Medical Centre",
            description: "Well Kids Medical Centre is dedicated to providing exceptional pediatric healthcare services with a child-friendly approach. Our team of experienced pediatric professionals strives to deliver the highest quality care in a comfortable and welcoming environment.",
            mission: "Our Mission",
            mission_text: "To provide exceptional healthcare services with compassion and expertise.",
            vision: "Our Vision",
            vision_text: "To be the leading healthcare provider, recognized for excellence in patient care."
        },
        contact: {
            title: 'Contact Us',
            intro: "We're here to help with your child's healthcare needs. Feel free to reach out to us through any of the following channels:",
            address: {
                title: 'Address',
                value: "Room 807, Argyle Centre, 688 Nathan Road, Mong Kok, Kowloon"
            },
            phone: {
                title: 'Phone',
                value: '+852 23213318'
            },
            email: {
                title: 'Email',
                value: 'wellkidsmc@gmail.com'
            },
            hours: {
                title: 'Operating Hours',
                value: 'Monday - Friday: 10:00 AM - 6:00 PM\nSaturday: 10:00 AM - 1:00 PM\nSunday and Public Holiday: Closed'
            }
        }
    },
    zh: {
        vaccine: {
            title: "兒童疫苗計劃",
            intro: "兒童醫療中心提供全面的兒童疫苗接種服務，確保您的孩子獲得完整的疫苗保護。以下是我們的疫苗接種計劃，包括政府健康院疫苗接種計劃和建議的疫苗接種計劃-加強保護。",
            importance_title: "疫苗接種的重要性",
            importance_text1: "疫苗接種是預防疾病最有效的方法之一，對兒童尤為重要。通過接種疫苗，有助於兒童的免疫系統識別和抵抗特定病原體，從而預防各種嚴重疾病。",
            importance_text2: "我們的兒科專家將根據您孩子的年齡、健康狀況和需求，提供個性化的疫苗接種建議。如有任何疑問，請隨時與我們聯繫。",
            table: {
                age_group: "年齡組別",
                govt_program: "政府健康院疫苗計劃",
                enhanced_program: "建議的疫苗計劃-加強保護",
                newborn: "初生",
                one_month: "一個月",
                two_months: "兩個月",
                four_months: "四個月",
                six_months: "六個月",
                nine_months: "九個月",
                one_year: "一歲",
                fifteen_months: "十五個月",
                eighteen_months: "十八個月",
                two_years: "兩歲",
                bcg: "卡介苗",
                hepb: "乙型肝炎疫苗",
                dtap_ipv: "四合一疫苗",
                pcv: "肺炎球菌疫苗",
                mmr: "麻疹、腮腺炎、德國麻疹混合疫苗",
                varicella: "水痘疫苗",
                six_in_one: "六合一疫苗",
                five_in_one: "五合一疫苗",
                rotavirus: "輪狀病毒疫苗",
                je: "日本腦炎疫苗",
                menb: "腦膜炎雙球菌疫苗 B型",
                mmrv: "麻疹、腮腺炎、德國麻疹及水痘混合疫苗",
                menc: "腦膜炎雙球菌疫苗 ACWY型",
                hepa: "甲型肝炎疫苗",
                booster: "加強劑",
                first_dose: "第1次",
                second_dose: "第2次",
                third_dose: "第3次",
                fourth_dose: "第4次",
                included: "(不用打)，已包含在六合一疫苗"
            }
        },
        footer: {
            address: {
                title: "地址",
                street: "旺角彌敦道688號旺角中心807室"
            },
            appointment: {
                title: "預約查詢",
                phone: "+852 23213318",
                email: "wellkidsmc@gmail.com"
            },
            social: {
                title: "關注我們",
                facebook: "Facebook",
            instagram: "Instagram",
            twitter: "Twitter",
            whatsapp: "WhatsApp掃碼"
            }
        },
        about: {
            title: "關於允翹兒科醫務中心",
            description: "允翹兒科醫務中心致力於以兒童友好的方式提供卓越的兒科醫療服務。我們經驗豐富的兒科專業團隊努力在舒適和溫馨的環境中提供最優質的護理。",
            mission: "我們的使命",
            mission_text: "以同理心和專業知識提供卓越的醫療服務。",
            vision: "我們的願景",
            vision_text: "成為以患者護理卓越著稱的領先醫療服務提供者。"
        },
        home: {
            hero: {
                title: "允翹兒科醫務中心",
                subtitle: "允翹兒科醫務中心提供兒科專科門診服務、新生嬰兒黃疸治療、緊急分娩到診服務、初生嬰兒住院巡房服務 (浸會醫院、聖德肋撒醫院(法國醫院))、私家醫院住院安排 ，聖德肋撒醫院(法國醫院)"
            },
            features: {
                expertise: {
                    title: "醫療專長",
                    description: "我們經驗豐富的醫生團隊提供全面的醫療服務。"
                },
                care: {
                    title: "患者護理",
                    description: "我們注重患者舒適度和個性化治療方案。"
                },
                facilities: {
                    title: "現代設施",
                    description: "先進的醫療設備和舒適的環境。"
                }
            }
        },
        services: {
            title: "兒科服務",
            description: "我們有多種兒童醫療服務，包括：",
            general_medicine: "香港兒科專科門診服務",
            general_medicine_desc: "發展評估、入學身體檢查、過敏檢測、脫敏治療、發燒、疾病。",
            pediatrics: "疫苗接種",
            pediatrics_desc: "新生嬰兒、外國出生嬰兒補打香港需要疫苗、外國升學疫苗接種。提供衞生署指定嬰幼兒及適齡學童接種疫苗及額外預防針，如日本腦炎、甲型肝炎、腦膜炎、輪狀病毒、乙型嗜血性流感桿菌等。",
            cardiology: "母乳顧問",
            cardiology_desc: "由擁有母乳餵哺知識和豐富指導經驗的國際認證泌乳顧問，輔助新手媽媽以正確姿勢餵哺母乳。",
            dermatology: "言語治療",
            dermatology_desc: "由言語治療師為有語言發展遲緩、發音或聲線障礙、口部肌肉等問題的兒童提供評估與治療。",
            orthopedics: "住院服務",
            orthopedics_desc: "初生嬰兒住院巡房服務，私家醫院住院安排。"
        },
        nav: {
            home: '首頁',
            about: '關於我們',
            services: '服務',
            doctors: '醫生團隊',
            contact: '聯絡我們'
        },
        doctors: {
            title: '我們的醫療團隊',
            doctor1: {
                name: '蘇詠怡醫生',
                specialty: '兒科專科醫生',
                credentials: '香港大學内外全科醫學士\n英國皇家兒科醫學院院員\n香港兒科醫學院院士\n香港醫學專科學院院士（兒科）\n英國皇家兒科醫學院兒科文憑（國際）\n香港兒科醫學院兒科文憑（香港）'
            },
            doctor2: {
                name: '曾志豪醫生',
                specialty: '兒科專科醫生',
                credentials: '香港大學内外全科醫學士\n英國皇家兒科醫學院院員\n香港兒科醫學院院士\n香港醫學專科學院院士（兒科）\n英國皇家兒科醫學院兒科文憑（國際）\n香港兒科醫學院兒科文憑（香港）'
            }
        },
        contact: {
            title: '聯絡我們',
            intro: '我們隨時為您的孩子提供醫療保健服務。您可以透過以下方式聯絡我們：',
            address: {
                title: '地址',
                value: '九龍旺角彌敦道688號旺角中心807室'
            },
            phone: {
                title: '電話',
                value: '+852 23213318'
            },
            email: {
                title: '電子郵件',
                value: 'wellkidsmc@gmail.com'
            },
            hours: {
                title: '營業時間',
                value: '星期一至五：上午10:00 - 下午6:00\n星期六：上午10:00 - 下午1:00\n星期日及公眾假期：休息'
            }
        }
    }
};
